...
export default {
  ...
  props: {
    ...
    // 上传模式：chunk(分片), direct(直接上传), supplement(补录)
    uploadMode: {
      type: String,
      default: 'chunk',
      validator: value => ['chunk', 'direct', 'supplement'].includes(value)
    },
    // 自定义参数
    customParams: {
      type: Object,
      ...
      opts: {
        ...
        target: (file, chunk) => {
          ...
          if (this.uploadMode === 'direct') {
            // 直接上传模式，不走分片
            return `api/fileUploadTask/directUpload`;
          } else if (this.uploadMode === 'supplement') {
            // 补录模式
            return `api/fileUploadTask/supplementUpload`;
          } else {
            // 默认分片模式
            // 分片检查请求
        ...
        testChunks: this.uploadMode === 'chunk', // 只有分片模式才测试分片
  ...
  methods: {
    ...
    customizeOptions(options) {
      if (this.uploadMode === 'direct' || this.uploadMode === 'supplement') {
        // 直接上传和补录模式不需要分片
