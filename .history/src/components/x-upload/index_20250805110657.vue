...
export default {
  ...
  methods: {
    ...
    async onFileAdded(file) {
      ...
      // 如果是自动模式，根据文件大小动态配置上传选项
      if (this.internalUploadMode === 'auto') {
        this.customizeOptions(this.internalCustomParams || {}, file);

        // 根据文件大小显示上传模式提示
        const actualMode = this.getActualUploadMode(file);
        const sizeText = this.formatFileSize(file.size);
        console.log(`文件 ${file.name} (${sizeText}) 将使用${actualMode === 'chunk' ? '分片' : '直接'}上传模式`);
      }
    ...
    // 获取文件的实际上传模式
    getActualUploadMode(file) {
      // 如果有补录相关参数，说明是补录上传
      if (this.internalCustomParams && this.internalCustomParams.supplementType) {
        return 'supplement';
      }

      // 如果是补录模式，直接返回
      if (this.internalUploadMode === 'supplement') {
        return 'supplement';
      }

      // 如果是自动模式，根据文件大小判断
      if (this.internalUploadMode === 'auto') {
        return file.size > this.fileSizeThreshold ? 'chunk' : 'direct';
      }

      // 其他情况返回原始模式
      return this.internalUploadMode;
    },
    ...
    // 自定义配置选项
    customizeOptions(options, file = null) {
      // 如果有文件，根据文件大小动态调整配置
      if (file && this.internalUploadMode === 'auto') {
        const actualMode = this.getActualUploadMode(file);
        if (actualMode === 'direct') {
          // 小文件直接上传
          this.opts.testChunks = false;
          this.opts.chunkSize = Infinity;
        ...
        // 非自动模式的原有逻辑
        const mode = this.internalUploadMode;
        if (mode === 'direct' || mode === 'supplement') {
          // 直接上传和补录模式不需要分片
          this.opts.testChunks = false;
